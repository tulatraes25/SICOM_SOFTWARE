...
const s = StyleSheet.create({
  ...
  tableRow: { flexDirection: 'row', paddingVertical: 4, borderBottomWidth: 0.5, borderColor: C.gray200 },
  tableRowAlt: { backgroundColor: C.gray50 },
  td: { fontSize: 8, color: C.gray800 },
  eventRow: { flexDirection: 'row', gap: 6, marginBottom: 4 },
  eventDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: C.green, marginTop: 3 },
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.content}>
            ...
            <View style={s.section}>
              ...
              {progress.map((p, i) => (
                <View key={i} style={i % 2 === 1 ? [s.tableRow, s.tableRowAlt] : s.tableRow}>
                  <View style={{ flex: 1 }}><Text style={s.td}>{p.note}</Text><Text style={{ fontSize: 7, color: C.gray400 }}>{p.tech?.full_name || 'Técnico'} — {fmtDateTime(p.created_at)}</Text></View>
                </View>
              ))}
            </View>
          )}
          ...
          {order.technicians && order.technicians.length > 0 && (
            <View style={s.infoCard}>
              <Text style={s.cardTitle}>Técnicos</Text>
            ...
            <View style={s.section}>
              ...
              {events.map((e, i) => (
                <View key={i} style={s.eventRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 8, color: C.gray800 }}>{EVENT_LABELS[e.event_type] || e.event_type}</Text>
                    {e.performer?.full_name && <Text style={{ fontSize: 7, color: C.gray400 }}>{e.performer.full_name}</Text>}
                    <Text style={{ fontSize: 7, color: C.gray400 }}>{fmtDateTime(e.created_at)}</Text>
                  </View>
                </View>
              ))}
            </View>

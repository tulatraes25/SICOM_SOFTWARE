...
import { getServiceOrder, startOrder, completeOrder, addProgress, getOrderProgress } from '@/services/serviceOrders.service';
...
const STATUS_BADGE: Record<string, 'default' | 'success' | 'warning' | 'danger' | 'info'> = { assigned: 'warning', in_progress: 'warning', visited: 'info', completed: 'success' };
...
export default function TechServiceOrderDetailPage() {
  ...
  const loadData = async () => {
    if (!id) return;
    try {
      const [o, prog] = await Promise.all([getServiceOrder(id), getOrderProgress(id)]);
      setOrder(o); setProgress(prog);
    ...
    <DashboardLayout role="technician" title={`Orden ${numLabel}`}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            {progress.length > 0 && <Card><CardHeader><h3 className="font-semibold">Avances</h3></CardHeader><CardContent>
              <div className="space-y-2">{progress.map((p) => <div key={p.id} className="p-2 border rounded text-sm"><p className="font-medium">{p.note}</p><p className="text-xs text-gray-500">{(p.tech as any)?.full_name} — {new Date(p.created_at).toLocaleString('es-AR')}</p></div>)}</div>
            </CardContent></Card>}
          </div>
          <div className="space-y-6">
      ...
      {showProgressModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="bg-white rounded-xl max-w-md w-full p-6">
        ...
        <select className="w-full border rounded px-3 py-2 text-sm mb-3" value={progressType} onChange={(e) => setProgressType(e.target.value)}>
          ...
          <option value="material_needed">Material necesario</option>
          <option value="completion_note">Nota de finalización</option>
        </select>
        <textarea className="w-full border rounded px-3 py-2 text-sm resize-none" rows={3} value={progressNote} onChange={(e) => setProgressNote(e.target.value)} placeholder="Descripción del avance..." />
